...
class Cluster(object):
    ...
    def fit(self, X, y=None):
        ...
        for sample, label in zip(X, y):
            ...
            if all(c is not None for c in clusters):
                ...
                if all(c == clusters[0] for c in clusters):
                    ...
                    cluster = clusters[0]
                    #added
                    #log_resource_utilization(f"Match found: Sample {sample} assigned to existing cluster {cluster.identifier}",data_processed=data_processed, start_time=start_time)

                # Case 1b: Destination and certificate -> different clusters
                else:
                    # Create new cluster
                    cluster = self.new_cluster()
                    #log_resource_utilization(f"New cluster created: {cluster.identifier} for sample {sample}. Merging existing clusters",data_processed=data_processed, start_time=start_time)
                    # For each match
                    for c in clusters:
                        # Add samples from old cluster to new cluster
                        cluster.merge(c)
                       # log_resource_utilization(f"Merged existing cluster {c.identifier} into new cluster {cluster.identifier}",data_processed=data_processed, start_time=start_time)
            ...
            # Case 2: Single or no matches
                ...
                cluster = cluster[0]
                #added upto else
                '''if cluster not in self.dict_certificate.values() and cluster not in self.dict_destination.values():
                    log_resource_utilization(f"New cluster created: {cluster.identifier} for sample {sample}",data_processed=data_processed, start_time=start_time)
                else:
                    log_resource_utilization(f"Match found: Sample {sample} assigned to existing cluster {cluster.identifier}",data_processed=data_pe=start_time)'''
        ...
        log_resource_utilization("After Cluster creation", data_processed=data_processed, start_time=start_time)
        # Return result
        return self
    ...
    def plot(self, annotate=False,output_filename="diag.png",output_path="ec2/user/FlowPrint/flowprint/graphs"):
