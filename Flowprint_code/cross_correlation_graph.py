...
class CrossCorrelationGraph(object):
    ...
    def fit(self, cluster, y=None):
        ...
        log_resource_utilization("After cross correlation computation",  start_time=start_time)
    ...
    def activity(self, cluster):
        ...
        return mapping
    def plot_and_save_graph(self, filename="graph.png", dense=True, output_dir="/home/ec2-user/FlowPrint/flowprint/graphs"):
        """Plot and save the CrossCorrelationGraph as an image."""
        os.makedirs(output_dir, exist_ok=True)
        full_path = os.path.join(output_dir, filename)

        graph = self.graph if dense else self.unpack()
    ...
    # Other methods (cross_correlation, activity, export, unpack) go here...

# Usage Example
# Assuming you have a 'cluster' object already defined with flows
# cluster = ...  # Your cluster object here
graph_instance = CrossCorrelationGraph(window=30, correlation=0.1)
#graph_instance.fit(cluster)  # Fit with your cluster data
graph_instance.plot_and_save_graph(filename="my_graph.png")  # Save graph after fitting
